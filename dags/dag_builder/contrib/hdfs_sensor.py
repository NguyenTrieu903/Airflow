# from airflow.sensors import HdfsSensor
    ...
    def wrap_func(obj, result, *args, **kwargs):
        ...
        try:
            ...
            if size < obj.min_size:
                logging.info("size = {actual} < {min_size} ~ {ratio}".format(actual=size, min_size=obj.min_size, ratio=str(float(size)/float(obj.min_size))))
                return []
            else:
                return result
